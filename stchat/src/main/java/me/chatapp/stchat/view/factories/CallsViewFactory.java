...
public class CallsViewFactory {
    ...
    public static VBox create(Runnable onBackClicked) {
        ...
        // Calls list
        VBox callsList = new VBox(5);
        callsList.setPadding(new Insets(10, 0, 0, 0));

        Label callInfo = new Label("📞 Call history will appear here");
        callInfo.setStyle("-fx-text-fill: #72767d; -fx-font-size: 12px;");

        callsList.getChildren().add(callInfo);

        content.getChildren().addAll(header, createSeparator(), callsList);
        return content;
    }
